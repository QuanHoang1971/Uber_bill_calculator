...
// sau này sửa đổi j thì thay đổi ở đây
...
function kiemTraLoaiXe() {
  ...
  // xem ng dùng chọn loại xe nào

  //   ng dùng tick chọn xe nào thì mới return ra đúng loại xe đó
  if (uberX.checked) {
    return "uberX";
...
function tinhTienCho(thoiGianCho, giaCho) {
  ...
  if (thoiGianCho >= 3) {
    ...
    // làm tròn trên Math.round. nếu 4ph sẽ tính 1 lần nhưng
    // 5' tính 2 lần
  }
  return tienCho;
}
...
function tinhTongTien() {
  ...
  switch (loaiXe) {
    case "uberX":
      ...
      break;
    //   sau mỗi switch nhớ dùng break để nếu đúng đk thì ko chạy xuống dưới nữa
    // đưa vào giá trị đã tạo ra ở trên
    case "uberSUV":
...
function renderRowChiTietKm(loaiXe, arrayKm, arrayPrice, tblBody) {
  ...
  for (var i = 0; i < arrayKm.length; i++) {
    ...
    // sau khi tạo var td..sẽ gán giá trị cho nó
...
function renderRowTongCong(tongTien, tblBody) {
  ...
  // set attribute tên j và gía trị của nó ntn
...
function inHoaDon(loaiXe, soKm, thoiGianCho, giaCho, arrayPrice, tongTien) {
  ...
  // vì khi ấn nút in hóa đơn sẽ tạo ra các bảng tr nếu ko reset thì
  // sẽ in liên tiếp, appendChild sẽ thêm tiếp vào cuối

  if (soKm <= 1) {
    renderRowChiTietKm(loaiXe, [1], arrayPrice, tblBody);
    //soKm đầu tiên là 1km nên chỉ có 1 phần tử
  } else if (soKm > 1 && soKm <= 20) {
    renderRowChiTietKm(loaiXe, [1, soKm - 1], arrayPrice, tblBody);
    // gồm 1km và số Km 1-19
  } else if (soKm > 20) {
